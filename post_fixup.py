...
fig, axs = plt.subplots(2, 2, height_ratios=[0.3,0.7], width_ratios=[0.7, 0.3], figsize=(5.45,3.8))
...
data.add_column(Table.Column(name='invalid_group_mean_sn', data=~(selection | selection_2)))
...
# data.write(f'/media/septagonic/CORSAIR/group_islands_selected_{sn_name}.fits', format='fits', overwrite=True)
data.write(f'/media/septagonic/CORSAIR/group_islands_selected_final.fits', format='fits', overwrite=True)
